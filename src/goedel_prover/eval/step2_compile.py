...
from goedel_prover.prover.lean.verifier import Lean4ServerScheduler
...
lean4_scheduler = Lean4ServerScheduler(
    max_concurrent_requests=args.cpu, timeout=300, memory_limit=10, name="verifier"
)
...
for i in range(len(codes)):
    ...
    ana_result.append(
        {"name": codes[i]["name"], "compilation_result": outputs_list[i]["complete"]}
    )
