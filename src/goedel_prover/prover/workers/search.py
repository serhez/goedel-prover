...
from goedel_prover.prover.utils import AttrDict, get_datetime


class SearchProcess(mp.Process):
    def __init__(
        self, idx, log_dir, tokenizer_path, scheduler, data_loader, sampler, model_args
    ):
        self.idx = idx
    ...
    def run(self):
        while True:
            ...
            self.process_print(
                "Success: {} / {}    Generation: {:.2f} secs    Verification: {:.2f} secs".format(
                    success_count,
                    len(candidate_list),
                    sample_timecost,
            ...
            for success_flag, summary_list in summary_dict.items():
                if len(summary_list) > 0:
                    ...
                    with open(
                        prob_log_basedir
                        / f"{success_flag}-{log_tag}-{get_datetime()}.json",
                        "w",
                    ) as json_file:
